...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
local defaults = {time = 0, maxCamOffset = 10, minHeading = 15, maxHeading = 165, minPitch = -10, maxPitch = 15, minDistance = 60, maxDistance = 80}
local MAX_TRIES = 50
local tau = 2*math.pi
...
local function BeautyShot(unitID, params)
  ...
  dist2d = dist2d * unitDef.xsize^0.5
  ...
  if WG.COFC then
    WG.COFC.SetThirdPersonTrackParams({dist = dist2d, heading = camHeading, pitch = camPitch})
    WG.COFC.SetThirdPersonTrackUnit(unitID, true)
  else
